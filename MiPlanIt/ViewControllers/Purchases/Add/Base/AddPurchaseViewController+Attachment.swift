//
//  AddPurchaseViewController+Attachment.swift
//  MiPlanIt
//
...
extension AddPurchaseViewController: AttachFileDropDownViewControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage, let compressedImage = image.fixOrientation().jpegData(compressionQuality: 0.5) {
            self.purchaseModel.addAttachement(UserAttachment(with: compressedImage, type: .purchase, ownerId: self.purchaseModel.ownerUserId))
