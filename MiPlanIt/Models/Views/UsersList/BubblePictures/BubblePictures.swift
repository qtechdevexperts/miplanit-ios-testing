...
public class BubblePictures: NSObject {
    ...
    public init(collectionView: UICollectionView, configFiles: [BPCellConfigFile], layoutConfigurator: BPLayoutConfigurator = BPLayoutConfigurator(), useCustomBubbleCount: Bool = false, callBack: BPDelegate) {
        self.useCustomBubbleCount = useCustomBubbleCount
    ...
    private func truncateCells(configFiles: [BPCellConfigFile]) {
        ...
        for (index, configFile) in configFiles.enumerated() {
            if index + 1 == maxNumberOfBubbles {
                ...
                case .none:
                    truncatedCell = BPCellConfigFile(BPImageType.color(layoutConfigurator.backgroundColorForTruncatedBubble), imageUrl: configFile.imageUrl,
                                                     title: "+\(remainingCells)", name: configFile.name, status: configFile.status, respondStatus: .eDefault
                    )
                case .some(.image(let imageType)):
                    truncatedCell = BPCellConfigFile(imageType, imageUrl: configFile.imageUrl,
                                                     title: "", name: configFile.name, status: configFile.status, respondStatus: .eDefault
                    )
                case .some(.number(let number)):
                    truncatedCell = BPCellConfigFile(BPImageType.color(layoutConfigurator.backgroundColorForTruncatedBubble), imageUrl: configFile.imageUrl,
                                                     title: "+\(number)", name: configFile.name, status: configFile.status, respondStatus: .eDefault
                    )
                case .some(.text(let text)):
                    truncatedCell = BPCellConfigFile(BPImageType.color(layoutConfigurator.backgroundColorForTruncatedBubble), imageUrl: configFile.imageUrl,
                                                     title: text, name: configFile.name, status: configFile.status, respondStatus: .eDefault
                    )
                }
    ...
    fileprivate var useCustomBubbleCount: Bool = false
    ...
    fileprivate var maxNumberOfBubbles: Int {
        if self.useCustomBubbleCount {
            if let maxNumberPreferredByUser = layoutConfigurator.maxNumberOfBubbles {
...
extension BubblePictures: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        guard let width = self.layoutConfigurator.cellWidth, let height = self.layoutConfigurator.cellHeight else {
