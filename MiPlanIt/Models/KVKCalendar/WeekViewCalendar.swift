...
final class WeekViewCalendar: UIView {
    ...
    func reloadData(events: [Event]) {
        data.events = events
        scrollHeaderDay.reloadData()
        timelineView.create(dates: visibleDates, events: events.filter({ event in return visibleDates.contains(where: { return compareStartEndDate(event: event, date: $0) }) }), selectedDate: data.date)
    }
...
extension WeekViewCalendar: CalendarSettingProtocol, CompareEventDateProtocol {
    func reloadFrame(_ frame: CGRect) {
        ...
        timelineView.create(dates: visibleDates, events: data.events.filter({ event in return visibleDates.contains(where: { return compareStartEndDate(event: event, date: $0) }) }), selectedDate: data.date)
...
extension WeekViewCalendar: TimelineDelegate {
    ...
    func didTimeLineChangedFinished(to dateTime: Date, with events: [Event]) {
        self.delegate?.didChangeTimeLine(dateTime, with: events, onDragging: false)
    }
