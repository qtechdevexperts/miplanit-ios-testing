...
final class MonthViewCalendar: UIView {
    ...
    private lazy var headerView: WeekHeaderView = {
        ...
        if style.month.isHiddenTitleDate {
            height = style.month.heightHeaderWeek + 10
        } else {
            height = style.month.heightHeaderWeek + style.month.heightTitleDate + 10
        }
...
extension MonthViewCalendar: CalendarSettingProtocol {
    func reloadFrame(_ frame: CGRect) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
            self.frame = frame
            self.headerView.reloadFrame(frame)
            
            self.collectionView.removeFromSuperview()
            self.collectionView = self.createCollectionView(frame: self.frame, style: self.style.month)
            
            var collectionFrame = frame
            collectionFrame.origin.y = self.headerView.frame.height
            collectionFrame.size.height = collectionFrame.height - self.headerView.frame.height
            self.collectionView.frame = collectionFrame
            self.addSubview(self.collectionView)
            
            if let idx = self.data.days.firstIndex(where: { $0.date?.month == self.data.date.month && $0.date?.year == self.data.date.year }) {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
