...
private let countInCell: CGFloat = 3
...
final class MonthCollectionViewCell: UICollectionViewCell {
    ...
    var events: [Event] = [] {
        didSet {
            ...
            for (idx, event) in events.enumerated() {
                ...
                let count = idx + 1
                let label = UILabel(frame: CGRect(x: (frame.width - height) / 2, y: dateLabel.bounds.height + (height) * CGFloat(idx), width: height, height: height))
                label.isUserInteractionEnabled = true
                if count > MonthCollectionViewCell.titlesCount {
                    label.font = UIFont(name: Fonts.SFUIDisplayRegular, size: 10)!
                    ...
                    label.text = "+\(events.count - MonthCollectionViewCell.titlesCount)"
                    label.layer.cornerRadius = (height) / 2
                    label.backgroundColor = UIColor(red: 74/255.0, green: 74/255.0, blue: 74/255.0, alpha: 1.0)
                    label.clipsToBounds = true
                    addSubview(label)
                    return
                } else {
                    if !event.isAllDay || UIDevice.current.userInterfaceIdiom == .phone {
                        label.attributedText = addIconBeforeLabel(eventList: [event],
                                                                  ...
                                                                  bulletAttributes: [.font: monthStyle.fontEventBullet, .foregroundColor: event.dotColor ],
                        ...
                        label.backgroundColor = event.dotColor
    ...
    @objc private func activateMoveEvent(gesture: UILongPressGestureRecognizer) {
        ...
        snapshotLabel.backgroundColor = event.backgroundColor
        snapshotLabel.attributedText = addIconBeforeLabel(eventList: [event],
                                                          textAttributes: [.font: monthStyle.fontEventTitle, .foregroundColor: event.colorText],
                                                          bulletAttributes: [.font: monthStyle.fontEventBullet, .foregroundColor: event.colorText],
                                                          timeAttributes: [.font: monthStyle.fontEventTime, .foregroundColor: event.colorText],
                                                          indentation: 0,
                                                          lineSpacing: 0,
                                                          paragraphSpacing: 0)
