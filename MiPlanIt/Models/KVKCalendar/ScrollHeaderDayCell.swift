...
private let heightDotView: CGFloat = 5
...
final class ScrollHeaderDayCell: UICollectionViewCell {
    ...
    private lazy var titleLabel: UILabel = {
        ...
        label.font =  UIFont(name: Fonts.SFUIDisplayRegular, size: 12)!//.systemFont(ofSize: 11)
    ...
    private lazy var dateLabel: UILabel = {
        let label = UILabel()
        label.textAlignment = .center
        label.font = UIFont(name: Fonts.SFUIDisplayRegular, size: 18)!//.systemFont(ofSize: 18)
  ...
  // ----
    private lazy var dotView: UIView = {
        ...
        view.backgroundColor = #colorLiteral(red: 86/255.0, green: 173/255.0, blue: 189/255.0, alpha: 1)
        return view
    }()
 //***
    ...
    var isDotViewVisible = true {
        didSet {
            self.dotView.isHidden = !isDotViewVisible
        }
    }
    ...
    override init(frame: CGRect) {
        ...
        var dotFrame = frame
        dotFrame.size.height = frame.height > heightDotView ? heightDotView : frame.height / 2
        dotFrame.size.width = heightDotView
        dotFrame.origin.y = dateFrame.height + 26
        dotFrame.origin.x = (frame.width / 2) - (dotFrame.width / 2)
        ...
        dateLabel.layer.cornerRadius = dateLabel.frame.width / 4
    ...
    private func populateCell(_ item: DayStyle) {
        guard item.day.type == .saturday || item.day.type == .sunday else {
            ...
            titleLabel.textColor = headerStyle.colorNameDay
