...
final class ScrollDayHeaderView: UIView {
    ...
    init(frame: CGRect, days: [Day], date: Date, type: CalendarType, style: Style) {
        ...
        var newFrame = frame
        newFrame.origin.x = 0
        newFrame.origin.y = self.style.headerScroll.heightTitleDate
        ...
        if !style.headerScroll.isHiddenTitleDate {
            titleLabel.frame = CGRect(x: 0, y: 0, width: frame.width - frame.minX, height: self.style.headerScroll.heightTitleDate)
            setDateToTitle(date)
    ...
    func setDaysofVisible(_ event:[Event], date:Date) {
        self.days = [Day(type: .tuesday, date: date, data: event)]
    }
    ...
    private func createCollectionView(frame: CGRect, isScrollEnabled: Bool) -> UICollectionView {
        ...
        collection.isPagingEnabled = self.daySelectionFromMonth ? false : true
        collection.showsHorizontalScrollIndicator = false
        collection.backgroundColor = style.headerScroll.colorBackground
        collection.delegate = self
        collection.dataSource = self
        collection.isScrollEnabled = self.daySelectionFromMonth ? false : isScrollEnabled
...
//        guard var indexPath = getMiddleIndexPath(), let middleDate = days[indexPath.row].date else { return } // previous line
        guard var indexPath = getMiddleIndexPath() else { return } //712522800.0    //New line changes
        guard let middleDate = days.last?.date else { return } //New line changes
        switch type {
        case .day:
            let minOffset = 4
            let maxOffset = 5
...
extension ScrollDayHeaderView: CalendarSettingProtocol {
    func reloadFrame(_ frame: CGRect) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.02) {
            ...
            self.titleLabel.frame.size.width = self.frame.size.width - self.frame.origin.x
            self.collectionView.removeFromSuperview()
            let newView = self.createCollectionView(frame: self.frame, isScrollEnabled: self.style.headerScroll.isScrollEnabled)
            newView.frame.origin.x = 0
            newView.frame.origin.y = self.style.headerScroll.heightTitleDate
            if !self.style.headerScroll.isHiddenTitleDate {
                newView.frame.size.height = self.frame.height - self.style.headerScroll.heightTitleDate
            }
            self.addSubview(newView)
            guard let scrollDate = self.getScrollDate(self.date),
                let idx = self.days.firstIndex(where: { $0.date?.year == scrollDate.year
                    && $0.date?.month == scrollDate.month
                    && $0.date?.day == scrollDate.day }) else { return }
            ...
            newView.reloadData()
            self.collectionView = newView
        }
    }
    
    func reloadData(daySelectionFromMonth: Bool = false) {
        self.daySelectionFromMonth = daySelectionFromMonth
    ...
    private func getMiddleIndexPath() -> IndexPath? {
        ...
        print(rect)
        print(collectionView.indexPathForItem(at: CGPoint(x: rect.midX, y: rect.midY)))
...
extension ScrollDayHeaderView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.isDotViewVisible = self.style.headerScroll.isDotNeeded && dataSource?.willExistEventOnDate(day.date) ?? true
        cell.isUserInteractionEnabled = self.daySelectionFromMonth ? cell.isDotViewVisible : true
        cell.alpha = self.daySelectionFromMonth && !cell.isDotViewVisible ? 0.5 : 1.0
        return cell
    }
}
