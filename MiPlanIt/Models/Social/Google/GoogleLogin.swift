...
extension SocialManager {
    ...
    public func loginGoogleFromViewController(_ viewController: UIViewController, client: String, scopes: [String]? = nil, result: SocialManagerDelegate?) {
      ...
      let signInConfig = GIDConfiguration(clientID: client)
...
//        }
      ...
      GIDSignIn.sharedInstance.signIn(withPresenting: viewController, hint: nil, additionalScopes: scopes) { [weak self] user, error in
        guard let strongSelf = self else {return}
        if error != nil {
          strongSelf.sendGoogleAccountsToServerFailed()
        } else {
          guard let googleUser = user?.user else {
            strongSelf.delegate?.socialManagerFailedToLogin(strongSelf)
            return
          }
          let socialUser = SocialUser(with: googleUser)
          strongSelf.sendGoogleAccountsToServer([socialUser])
          strongSelf.delegate?.socialManager(strongSelf, loginWithResult: socialUser)
        }
      }
    }
    ...
    func refreshAccountWithResult(_ currentUser: GIDGoogleUser, result: @escaping ([SocialUser]?) -> ()) {
      currentUser.refreshTokensIfNeeded { auth, error in
