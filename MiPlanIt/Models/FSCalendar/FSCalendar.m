...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        self.calendarWeekdayView.frame = CGRectMake(0, self.calendarHeaderView.fs_bottom + 5, self.contentView.fs_width, weekdayHeight);
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    if (shouldTriggerPageChange) {
        ...
        [self didChangeValueForKey:@"currentPage"];
       // [self setCurrentDateAfterScrolling];
    }
    
}

-(void)setCurrentDateAfterScrolling {
    if (self.scope == FSCalendarScopeWeek && self.selectedDate != nil) {
        NSDateComponents *dayComponent = [[NSDateComponents alloc] init];
        dayComponent.day = 7;
        NSDate *nextDate = [self.gregorian dateByAddingComponents:dayComponent toDate:self.selectedDate options:0];
        [self selectDate:nextDate];
    }
}
...
//            [_contentView addSubview:calendarWeekdayView];
            [_contentView insertSubview:calendarWeekdayView belowSubview:_collectionView];
            self.calendarWeekdayView = calendarWeekdayView;
