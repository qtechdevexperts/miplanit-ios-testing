...
open class CardStackViewLayout: UICollectionViewLayout, UIGestureRecognizerDelegate {
    ...
    @IBInspectable open var topStackMaximumSize: Int = 4
    
    /// The maximum number of cards displayed on the bottom stack.
    @IBInspectable open var bottomStackMaximumSize: Int = 4
    ...
    @IBInspectable open var verticalOffsetBetweenCardsInTopStack: Int = 8
    @IBInspectable open var centralCardYPosition: Int = 35
...
//                index += 1
//            }
            self.delegate?.cardStackViewLayoutSwipeDown(self)
        default:
            break
        }
    }
    ...
    fileprivate func findDraggingCellByCoordinate(_ touchCoordinate: CGPoint) {
        if let indexPath = collectionView?.indexPathForItem(at: touchCoordinate) {
            ...
            if let indexPath = draggedCellPath {
                initialCellCenter = collectionView?.cellForItem(at: indexPath)?.center
    ...
    fileprivate func updateCenterPositionOfDraggingCell(_ touchCoordinate:CGPoint) {
        if let strongDraggedCellPath = draggedCellPath {
            if let cell = collectionView?.cellForItem(at: strongDraggedCellPath), let initialCellCenter = initialCellCenter {
    ...
    fileprivate func finishedDragging(_ cell: UICollectionViewCell) {
        guard var initialCellCenter = initialCellCenter else { return }
    ...
    func newCardDidAdd(_ newCardIndex: Int) {
        collectionView?.performBatchUpdates({ [weak self] () in
            ...
            self?.collectionView?.reloadItems(at: [IndexPath(row: 3, section: 0), IndexPath(row: 2, section: 0), IndexPath(row: 1, section: 0), IndexPath(row: 0, section: 0)])
            }, completion: {[weak self] _ in
